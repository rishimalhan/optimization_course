...
class DFP
{
	...
	public:
		...
		Eigen::VectorXd optimize( int _dimension, double _alpha, Eigen::VectorXd _x_c, double _tol, int _type_search )	
		{
			...
			Eigen::MatrixXd H = Eigen::MatrixXd::Identity(dimension,dimension);
			...
			{
				iter_count += 1;
				if(iter_count!=1)	{ H = update_H( H,dx,dg ); }
				dir_vec = H * (-g_p);
			...
			std::cout<< "Quasi Newton DFP Completed. Number of Iterations:  " << iter_count << "\n" << std::endl;
			return x_c;
		}
};
#endif